...
public class ShowtimesController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateShowtime([FromBody] CreateShowtimeRequest request)
    {
        ...
        try
        {
            ...
            var movieData = await _cacheService.GetOrSetAsync(
                $"movie_{request.MovieId}",
                async () => await _providedApiClient.GetMovieAsync(request.MovieId),
                TimeSpan.FromHours(1));

            var showtime = await _showtimesRepository.CreateShowtimeAsync(
                request.MovieId,
    ...
    public async Task<IActionResult> ReserveSeats(int showtimeId, [FromBody] ReserveSeatsRequest request)
    {
        ...
        try
        {
            ...
            var showtime = await _showtimesRepository.GetShowtimeAsync(showtimeId);
            if (showtime == null)
            ...
            var availableSeats = await _ticketsRepository.GetAvailableSeatsAsync(showtimeId);
    ...
    public async Task<IActionResult> ConfirmReservation(Guid reservationId)
    {
        ...
        try
        {
            var reservation = await _ticketsRepository.GetReservationAsync(reservationId);
            if (reservation == null)
            ...
            var ticket = await _ticketsRepository.ConfirmReservationAsync(reservationId);
